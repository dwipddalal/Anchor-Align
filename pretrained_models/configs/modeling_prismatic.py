...
class OpenVLAForActionPrediction(PrismaticForConditionalGeneration):
    ...
    def _regression_or_discrete_prediction(
        ...
        # When using action_head, we only need hidden_states (not logits).
        # Bypass lm_head to save ~11 GB GPU memory at batch=32 (avoids
        # vocab_size projection + float32 cast that causes OOM on A100-40GB).
        if action_head is not None:
            # Call the inner model directly (Qwen2Model), skipping lm_head
            from transformers.modeling_outputs import BaseModelOutputWithPast
            inner_output = self.language_model.model(
                input_ids=None,
                attention_mask=multimodal_attention_mask,
                position_ids=None,
                past_key_values=None,
            ...
            # Wrap to match CausalLMOutputWithPast interface used below
            class _HiddenStatesOnly:
                def __init__(self, hidden_states):
                    self.hidden_states = hidden_states
                    self.logits = None
            language_model_output = _HiddenStatesOnly(inner_output.hidden_states)
        else:
            language_model_output = self.language_model(
                input_ids=None,
                attention_mask=multimodal_attention_mask,
    ...
    def predict_action(
        ...
        **kwargs: str,
        ...
        if use_proprio:
            proprio = np.atleast_2d(proprio)  # Handle both single (8,) and batched (N, 8)
        ...
        # Squeeze batch dim for backward compatibility with batch_size=1 callers
        if actions.shape[0] == 1:
            actions = actions[0]  # (chunk, dim) for backward compat
