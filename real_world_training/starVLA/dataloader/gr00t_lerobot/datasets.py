...
class LeRobotSingleDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Image size emitted by _pack_sample (fed to the VLM). Defaults to (224, 224) so
        # datasets that don't set it are unchanged; QwenGR00T real-world configs set
        # datasets.vla_data.obs_image_size: [448, 448] for hi-res (448 -> 256 vision tokens/image).
        # Without this, _pack_sample crushed every frame to 224 and the model later upscaled
        # 224 -> 448, wasting the hi-res token budget.
        _obs_size = self.data_cfg.get("obs_image_size", None) if self.data_cfg is not None else None
        self._pack_image_size = tuple(_obs_size) if _obs_size else (224, 224)
        ...
        # ---- Alignment-v7 EEF cache (optional) ---------------------------
        # If `data_cfg.eef_xyz_delta_cache` is set, load the precomputed FK
        # Cartesian XYZ-delta cache and build a trajectory_id -> per-frame
        # (T, 3) delta lookup. _pack_sample then attaches a chunk slice to
        # each sample as `eef_xyz_delta`, which alignment-v7 in the framework
        # consumes for direction-word supervision. No-op when the field is
        # absent or the load fails (alignment v7 then silently has no labels).
        self._eef_deltas_by_trajid: dict = {}
        self._eef_action_horizon: int = 1
        try:
            self._load_eef_xyz_delta_cache()
        except Exception as _e:
            print(
                f"[eef_cache] WARNING: load failed ({type(_e).__name__}: {_e}); "
                f"alignment v7 will be inactive for this dataset."
            )

    def _load_eef_xyz_delta_cache(self) -> None:
        """Load the precomputed FK Cartesian-XYZ-delta cache and match it to this
        dataset's trajectories via sha256(first_10 observation.state)[:16] fingerprint.
        Populates `self._eef_deltas_by_trajid[trajectory_id] -> (T, 3) float32` and
        `self._eef_action_horizon` (chunk length used to slice per-sample deltas).
        Skipped when `data_cfg.eef_xyz_delta_cache` is unset.
        """
        if self.data_cfg is None:
            return
        cache_path = self.data_cfg.get("eef_xyz_delta_cache", None)
        if not cache_path:
            return
        import pickle, hashlib, glob
        import pandas as pd
        with open(cache_path, "rb") as f:
            raw_cache = pickle.load(f)
        if not isinstance(raw_cache, dict):
            print(f"[eef_cache] unsupported cache format: {type(raw_cache).__name__}; expected dict.")
            return
        parquet_files = sorted(
            glob.glob(str(self._dataset_path / "data" / "chunk-*" / "file-*.parquet"))
        )
        if not parquet_files:
            print(f"[eef_cache] no data parquets under {self._dataset_path}/data; cannot fingerprint.")
            return
        all_df = pd.concat(
            [pd.read_parquet(f, columns=["episode_index", "observation.state"])
             for f in parquet_files],
            ignore_index=True,
        )
        matched = missed = 0
        for traj_id in self.trajectory_ids:
            sub = all_df[all_df["episode_index"] == int(traj_id)]
            if len(sub) < 10:
                missed += 1; continue
            states_first10 = np.stack(
                [np.asarray(v, dtype=np.float32) for v in sub["observation.state"].iloc[:10]],
                axis=0,
            )
            fp = hashlib.sha256(states_first10.astype(np.float32).tobytes()).hexdigest()[:16]
            entry = raw_cache.get(fp)
            if entry is None:
                missed += 1; continue
            self._eef_deltas_by_trajid[int(traj_id)] = np.asarray(entry, dtype=np.float32).reshape(-1, 3)
            matched += 1
        print(
            f"[eef_cache] {cache_path}: {len(raw_cache)} entries; "
            f"matched {matched}/{len(self.trajectory_ids)} trajectories (missed {missed})."
        )
        # Chunk horizon for slicing per-sample deltas, derived from action modality config.
        action_mod = self.modality_configs.get("action") if hasattr(self, "modality_configs") else None
        if action_mod is not None and getattr(action_mod, "delta_indices", None) is not None:
            self._eef_action_horizon = max(1, len(action_mod.delta_indices))
    ...
    def _pack_sample(self, data: dict, trajectory_id=None, base_index=None) -> dict:
        ...
        Optional `trajectory_id` and `base_index` are used to attach a per-sample
        `eef_xyz_delta` (chunk slice from the precomputed FK cache) for alignment v7.
        Callers that don't pass them simply omit that key (no-op for alignment).
        """
        step_images = []
        for video_key in self.modality_keys["video"]:
            image = data[video_key][0]
            image = Image.fromarray(image).resize(self._pack_image_size)
        ...
        # Alignment v7: attach the precomputed FK Cartesian XYZ deltas for this
        # sample's action chunk. No-op when the cache wasn't loaded (alignment off
        # or this trajectory wasn't matched), or when the caller didn't pass the
        # trajectory_id/base_index needed to slice the cache.
        if self._eef_deltas_by_trajid and trajectory_id is not None and base_index is not None:
            deltas = self._eef_deltas_by_trajid.get(int(trajectory_id))
            if deltas is not None:
                horizon = self._eef_action_horizon
                start = int(base_index)
                end = min(start + horizon, deltas.shape[0])
                chunk = deltas[start:end]
                if chunk.shape[0] < horizon:
                    pad = np.zeros((horizon - chunk.shape[0], 3), dtype=chunk.dtype)
                    chunk = np.concatenate([chunk, pad], axis=0)
                sample["eef_xyz_delta"] = chunk.astype(np.float32)   # (horizon, 3)
