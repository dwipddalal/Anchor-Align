...
class CalvinDataConfig(Libero4in1DataConfig):
    # CALVIN uses 64-step action chunks (at64) to match framework action_horizon=64.
    # Episodes are <=64 frames (median 64); the loader pads short ones.
    action_indices = list(range(64))
    # Custom fine-tuned franka embodiment. This is GR00T transform METADATA only
    # (server stores embodiment_tag.value); action un-normalization uses the
    # dataset_statistics.json key 'franka' (unnorm_key), NOT this tag — so this
    # is safe and does not affect normalization. Required by PolicyNormProcessor.
