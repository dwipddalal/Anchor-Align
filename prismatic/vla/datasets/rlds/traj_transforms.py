...
def chunk_act_obs(traj: Dict, window_size: int, future_action_window_size: int = 0) -> Dict:
    ...
    # Chunk eef_xyz_delta identically to action (if present)
    if "eef_xyz_delta" in traj:
        traj["eef_xyz_delta"] = tf.gather(traj["eef_xyz_delta"], floored_action_chunk_indices)
