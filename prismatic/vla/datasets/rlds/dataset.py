...
def make_dataset_from_rlds(
    ...
    *,
    ...
    def restructure(traj):
        ...
        # Save eef_xyz_delta before dict rebuild (injected by default_task_dataset_transform)
        _eef_xyz_delta = traj.get("eef_xyz_delta")
        ...
        # Restore eef_xyz_delta (precomputed EEF Cartesian deltas for alignment)
        if _eef_xyz_delta is not None:
            traj["eef_xyz_delta"] = _eef_xyz_delta
...
def make_interleaved_dataset(
    ...
    *,
    ...
    primary_dataset_indices = np.array(
        [idx for idx in range(len(sample_weights)) if sample_weights[idx] == 1.0], dtype=np.intp
    )
    # Fallback: if no weight is exactly 1.0, treat all datasets as primary
    if len(primary_dataset_indices) == 0:
        primary_dataset_indices = np.arange(len(sample_weights), dtype=np.intp)
    ...
    dataset = dataset.shuffle(shuffle_buffer_size, seed=seed, reshuffle_each_iteration=False)
