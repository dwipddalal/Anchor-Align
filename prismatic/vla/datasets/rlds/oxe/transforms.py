...
_EEF_DELTA_CACHE = None


def _load_eef_cache():
    """Load precomputed EEF XYZ delta cache (lazy singleton)."""
    global _EEF_DELTA_CACHE
    if _EEF_DELTA_CACHE is None:
        cache_path = os.path.join(
            os.path.dirname(__file__), "..", "..", "..", "..", "..", "data", "real_world", "eef_deltas_cache.pkl"
        )
        cache_path = os.path.normpath(cache_path)
        if os.path.exists(cache_path):
            with open(cache_path, "rb") as f:
                _EEF_DELTA_CACHE = pickle.load(f)
    return _EEF_DELTA_CACHE


def default_task_dataset_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    # Capture raw states BEFORE normalization for EEF delta fingerprint matching
    raw_states = trajectory["observation"]["state"]

    # Keep joint-angle channels unchanged; map gripper mm (800=open, 0=close) to [0, 1].
    gripper_action = tf.clip_by_value(trajectory["action"][:, -1:] / 800.0, 0.0, 1.0)
    trajectory["action"] = tf.concat([trajectory["action"][:, :-1], gripper_action], axis=1)

    gripper_state = tf.clip_by_value(trajectory["observation"]["state"][:, -1:] / 800.0, 0.0, 1.0)
    trajectory["observation"]["state"] = tf.concat(
        [trajectory["observation"]["state"][:, :-1], gripper_state],
        axis=1,
    )

    # Inject precomputed EEF XYZ deltas for alignment (real-world joint-space datasets)
    cache = _load_eef_cache()
    if cache is not None:
        def _lookup_eef_deltas(raw_st):
            raw_np = raw_st.numpy()
            fingerprint = hashlib.sha256(raw_np[:10].astype(np.float32).tobytes()).hexdigest()[:16]
            if fingerprint in cache:
                return cache[fingerprint].astype(np.float32)
            return np.zeros((raw_np.shape[0], 3), dtype=np.float32)

        eef_deltas = tf.py_function(_lookup_eef_deltas, [raw_states], tf.float32)
        eef_deltas.set_shape([None, 3])
        trajectory["eef_xyz_delta"] = eef_deltas
...
OXE_STANDARDIZATION_TRANSFORMS = {
    ...
    ### Real-world xArm7 vegetable dataset
    "default_task": default_task_dataset_transform,
    ### Real-world xArm7 clutter dataset (multi-task) — same pipeline as default_task
    "clutter_task": default_task_dataset_transform,
