...
# Lazy import tensorflow_graphics - only needed for DROID wrist transforms, not LIBERO
tfg = None

def _get_tfg():
    global tfg
    if tfg is None:
        import tensorflow_graphics.geometry.transformation as _tfg
        tfg = _tfg
    return tfg


def rmat_to_euler(rot_mat):
    return _get_tfg().euler.from_rotation_matrix(rot_mat)


def euler_to_rmat(euler):
    return _get_tfg().rotation_matrix_3d.from_euler(euler)


def invert_rmat(rot_mat):
    return _get_tfg().rotation_matrix_3d.inverse(rot_mat)
