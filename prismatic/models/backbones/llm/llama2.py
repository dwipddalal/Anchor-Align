...
# === Optional ungated-mirror override ===
# Setting `LLAMA2_HF_MIRROR=NousResearch` redirects all `meta-llama/Llama-2-*` paths above to the
# corresponding `NousResearch/Llama-2-*` mirrors. Useful for HF accounts without access to the
# gated meta-llama repos. Original LLAMA2_MODELS dict above is left untouched when env var unset.
_LLAMA2_MIRROR = os.environ.get("LLAMA2_HF_MIRROR", "").strip()
if _LLAMA2_MIRROR:
    for _k, _v in LLAMA2_MODELS.items():
        if _v["hf_hub_path"].startswith("meta-llama/"):
            _v["hf_hub_path"] = _v["hf_hub_path"].replace("meta-llama/", f"{_LLAMA2_MIRROR}/")
...
class LLaMa2LLMBackbone(HFCausalLLMBackbone):
    def __init__(
        ...
        # Allow disabling FlashAttention-2 via env var (e.g., on aarch64/GH200 where the
        # `flash_attn` wheel may not be installable). Original default `True` is preserved
        # when the env var is unset; setting `DISABLE_FLASH_ATTN_2=1` flips to False.
        if os.environ.get("DISABLE_FLASH_ATTN_2", "").strip().lower() in ("1", "true", "yes"):
