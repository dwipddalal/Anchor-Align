...
class PaddedCollatorForActionPrediction:
    ...
    def __call__(self, instances: Sequence[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        ...
        # FAST token labels for auxiliary CE loss (all same length = NUM_TOKENS)
        if "fast_token_labels" in instances[0]:
            output["fast_token_labels"] = torch.stack([instance["fast_token_labels"] for instance in instances])
